...
function CostTrend() {
  ...
  return <svg className="cost-chart" viewBox="0 0 720 230" role="img" aria-label="Month-to-date estimated cost rising from $419 to $4,137">
    ...
    {points.map(point=><g key={point.day}><circle cx={point.x} cy={point.y} r="3.5" fill="white" stroke="#316cf4" strokeWidth="2"/><text x={point.x} y="216" textAnchor="middle">{point.day}</text><title>{point.day}: {money.format(point.cost)}</title></g>)}
